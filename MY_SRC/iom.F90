...
!set which fields are to be written to a restart file
      ...
      !
      ! end file definition
              IF(lwp) WRITE(numout,*) 'HERE1g'
              CALL FLUSH(numout )
      dtime%second = rdt
      CALL xios_set_timestep( dtime )
              IF(lwp) WRITE(numout,*) 'HERE1h'
              CALL FLUSH(numout )
      CALL xios_close_context_definition()
              IF(lwp) WRITE(numout,*) 'HERE1i'
              CALL FLUSH(numout )
